...
export default function RatioHeatmap() {
  ...
  const wardData = wards.map((w) => {
    ...
    return {
      ...
      ratio,
      status,
      threshold: w.safe_ratio_threshold,
    };
  });
        ...
        >
          ...
          <div className="flex items-center justify-between text-xs">
            <span className="flex items-center gap-1">
              <TrendingUp className="w-3 h-3" /> {w.nurses} nurses
            </span>
            <span className="flex items-center gap-1">
              <TrendingDown className="w-3 h-3" /> {w.patients} patients
            </span>
          </div>
          ...
          <div className="mt-3 h-2 rounded-full bg-background/50 overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                w.status === "safe" ? "bg-emerald-500" : "bg-red-500"
              }`}
