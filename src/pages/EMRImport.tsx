...
const patientRowSchema = z.object({
  ...
  discharge_date: z.string().trim().optional(),
...
export default function EMRImport() {
    ...
    data.forEach((row, i) => {
      ...
      if (!result.success) {
        result.error.issues.forEach((issue) => {
          errs.push({ row: i + 2, field: issue.path.join("."), message: issue.message });
        });
      } else {
        valid++;
      }
      ...
      try {
        if (target === "patients") {
          ...
          const { error } = await supabase.from("patients").insert({
            ...
            ward_id: wardId,
            admission_date: pd.admission_date || new Date().toISOString(),
            discharge_date: pd.discharge_date || null,
            date_of_birth: pd.date_of_birth || null,
            gender: pd.gender || "Unknown",
            blood_type: pd.blood_type || null,
            diagnosis: pd.diagnosis || null,
            insurance_id: pd.insurance_id || null,
            emergency_contact: pd.emergency_contact || null,
            medical_record_number: pd.medical_record_number || null,
            import_source: "csv",
          ...
          const { error } = await supabase.from("nurses").insert({
            nurse_code: nd.nurse_code,
            full_name: nd.full_name,
            role_title: nd.role_title || "Nurse",
            ward_id: wardId,
            shift: nd.shift || "Day",
            status: nd.status || "On-Duty",
            license_number: nd.license_number || null,
            specialization: nd.specialization || null,
            years_of_experience: nd.years_of_experience ? parseInt(nd.years_of_experience) : 0,
            contact_number: nd.contact_number || null,
            import_source: "csv",
            imported_at: new Date().toISOString(),
          } as any);
          if (error) { failed++; batchErrors.push({ row: i + 2, error: error.message }); }
  ...
  const reset = () => {
    ...
    setValidCount(0);
    setFileName("");
    setImportResult({ inserted: 0, failed: 0 });
        ...
        {state === "idle" || state === "parsing" ? (
          <label className="flex flex-col items-center justify-center border-2 border-dashed border-border rounded-xl p-12 cursor-pointer hover:border-primary/50 hover:bg-muted/30 transition-colors">
            ...
            <span className="text-sm font-medium mb-1">
              {state === "parsing" ? "Parsing..." : "Click to upload CSV file"}
            </span>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-3 p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                ...
                <div>
                  <p className="text-sm font-medium">{validCount} Valid</p>
              ...
              <div className="flex items-center gap-3 p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                ...
                <div>
                  <p className="text-sm font-medium">{rows.length - validCount} Invalid</p>
            ...
            {errors.length > 0 && (
              <div className="max-h-48 overflow-y-auto bg-muted/50 rounded-lg p-3">
                <p className="text-xs font-medium text-muted-foreground mb-2">Validation Errors:</p>
                ...
                {errors.length > 50 && (
                  <p className="text-xs text-muted-foreground mt-2">...and {errors.length - 50} more errors</p>
                )}
              </div>
            )}
            ...
            <div className="overflow-x-auto max-h-64 overflow-y-auto">
              ...
              {rows.length > 10 && <p className="text-xs text-muted-foreground p-3">Showing 10 of {rows.length} rows</p>}
            </div>

            <div className="flex gap-3">
              <button onClick={doImport} disabled={validCount === 0} className="px-5 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50 transition">
                Import {validCount} Valid Records
              </button>
              <button onClick={reset} className="px-5 py-2.5 bg-muted text-muted-foreground rounded-lg text-sm font-medium hover:bg-accent transition">
                Cancel
              </button>
            </div>
          </div>
            ...
            <div className="flex items-center gap-3 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {importResult.inserted} inserted, {importResult.failed} failed — all changes are audit-logged
                </p>
              </div>
            </div>
            <button onClick={reset} className="px-5 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90 transition">
              Import Another File
            </button>
          </div>
        )}
